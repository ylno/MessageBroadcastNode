...
export class KonvBot {
	...
	async chatMessage(ctx: Context) {
		...
		if (text.toLowerCase() === 'help' || text === '/start') {
			await ctx.reply(this.getHelpText(),  {parse_mode: "HTML", ...Markup.keyboard(this.getMainMenuKeyboard()).resize()});
		} else if (text === 'LIST') {
			// Implement list logic
		} else if (text.toLowerCase() === 'new channel') {
			this.logger.debug("new channel request");
	...
	private async messageHandler(messageEvent: MessageEvent) {
		...
		try {
			const channel = await this.dataService.getChatDao().getChannel(messageEvent.channel);
